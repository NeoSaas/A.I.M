...
function PromptArea({ onSearch }) {
  ...
  const handleQuerySend = async (values) => {
    try {
      ...
      const response = await axios.post('http://127.0.0.1:8000/api/OPAICreateConvo/', values);
      const response_payload = response.data["response-payload"]
      await axios.post('http://127.0.0.1:8000/api/create-conversation-object', {response_payload, query})
    } catch (error) {
      console.error(error);
    }
  };
